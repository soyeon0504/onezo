...
export const ShopModal = ({ onCloseModal }) => {
  ...
  const drawMap = () => {
    ...
    var circle = new kakao.maps.Circle({
      center: new kakao.maps.LatLng(location.location_x, location.location_y), // 원의 중심좌표
    ...
    var distances = [];
    function calculateDistancesFromCurrentLocation(
      currentLat,
      currentLon,
      locations,
    ) {
    ...
    var distancess = calculateDistancesFromCurrentLocation(
      location.location_x,
      location.location_y,
      positions,
    ); // 리스트 안에 리스트를 담아서 배열형태가 아니라서 오류가 났던거다
    ...
    <div id="mapmap">
      ...
      <div className="scroll_y">
        <div className="store_list_main">
          {aroundMarker &&
            aroundMarker.map((position, index) => (
              <div className="store_list" key={index}>
                <img src="images/my/store.png" />
                <div
                  style={{ width: "970px", cursor: "pointer" }}
                  onClick={() => {
                    goLocation(position.latlng);
                  }}
                >
          ...
          {positions.map((position, index) => (
            <div className="store_list" key={index}>
              ...
              <div
                style={{ width: "970px", cursor: "pointer" }}
                onClick={() => {
                  navigate(`/shop?store=${position.id}`);
                  onCloseModal();
                }}
              >
                <p>{position.storeName}</p>
